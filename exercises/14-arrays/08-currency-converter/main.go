...
func main() {
	...
	const usage = "Usage: go run main.go <USD Amount>"
	convRatios := [...]float64{
		EUR: 0.88,
		GBP: 0.78,
		JPY: 113.02,
	...
	if err != nil {
		fmt.Println(usage)
		fmt.Println("Invalid amount. It should be a decimal number.")
		return
	}

	fmt.Printf("%.2f USD is %.2f EUR\n", usd, usd*convRatios[EUR])
	fmt.Printf("%.2f USD is %.2f GBP\n", usd, usd*convRatios[GBP])
	fmt.Printf("%.2f USD is %.2f JPY\n", usd, usd*convRatios[JPY])
