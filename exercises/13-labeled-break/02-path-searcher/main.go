...
func main() {
	corpus, _ := os.LookupEnv("PATH")
	paths := filepath.SplitList(corpus) // This uses OS Specific list separator so it should be platform independent.
	...
	for _, q := range query {
		...
		for i, path := range paths {
			if path == q || strings.Contains(path, q) {
				fmt.Printf("\n #%-2d: %q\n", i+1, path)
			}
		}
	}
