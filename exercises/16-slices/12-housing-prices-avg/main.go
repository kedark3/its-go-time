...
Istanbul,500,10,5,1000000`
	...
	fmt.Printf("\n%s\n", strings.Repeat("=", 75))
	var avgSize, avgBeds, avgBaths, avgPrice float64
	for i := range rows {
	...
	fmt.Printf("\n%s\n", strings.Repeat("=", 75))
	fmt.Printf("%-15s%-15.2f%-15.2f%-15.2f%-15.2f\n", "", avgSize/float64(len(sizes)), avgBeds/float64(len(beds)), avgBaths/float64(len(baths)), avgPrice/float64(len(prices)))
}
